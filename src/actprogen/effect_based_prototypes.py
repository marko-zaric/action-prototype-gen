...
class ActionPrototypes:
    def __init__(self, motion_samples: pd.DataFrame, motion_dimensions: list):
    ...
    def generate(self, effect_dimensions, prototypes_per_cluster=1):
        ...
        if len(effect_dimensions) == 1:
            ...
            kmeans = KMeans(n_clusters=best_num_of_clusters, random_state=0, n_init=10).fit(X)
    ...
    def single_prototype_per_class(self, cluster_label):
        ...
        if self.action_prototypes is None:
            ...
            self.action_prototypes = np.vstack((self.action_prototypes, cluster_action_means))
